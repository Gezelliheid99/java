                    ...
                    <div id="bottom">
                        <div>
                            <nav aria-label="..." style="text-align:center;">
                                <ul class="pagination">
                                    <li>
                                            <%--                                        <a href="${pageContext.request.contextPath}/prod/split.do?page=${info.prePage}" aria-label="Previous">--%>
                                    ...
                                    <c:forEach begin="1" end="${info.pages}" var="i">
                                        <c:if test="${info.pageNum==i}">
                                            <li>
                                                    <%--<a href="${pageContext.request.contextPath}/prod/split.do?page=${i}" style="background-color: grey">${i}</a>--%>
                                        ...
                                        <c:if test="${info.pageNum!=i}">
                                            <li>
                                                    <%--                                                <a href="${pageContext.request.contextPath}/prod/split.do?page=${i}">${i}</a>--%>
                                                <a href="javascript:ajaxsplit(${i})">${i}</a>
                                            </li>
                                        </c:if>
                                    </c:forEach>
                                    <li>
                                        <%--  <a href="${pageContext.request.contextPath}/prod/split.do?page=1" aria-label="Next">--%>
...
<script type="text/javascript">
    ...
    function deleteBatch() {
            ...
            if(zhi.length==0){
                ...
                //拼接ID
                    ...
                    //发送请求到服务器端
                   // window.location="${pageContext.request.contextPath}/prod/deletebatch.do?str="+str;
                    $.ajax({
                        url:"${pageContext.request.contextPath}/prod/deleteBatch.do",
                        data:{"pids":str},
                        type:"post",
                        dataType: "text",
    ...
    function del(pid,page) {
        if (confirm("确定删除吗")) {
          //向服务器提交请求完成删除
            ...
            var lowPrice = $("#lprice").val();
            var highPrice = $("#hprice").val();
            $.ajax({
                url:"${pageContext.request.contextPath}/prod/delete.do",
                data: {"pid":pid,"page":page,"pName":pName,"typeId":typeId,"lowPrice":lowPrice,"highPrice":highPrice},
                dataType:"text",
                type: "post",
                success:function(msg){
                    alert(msg);
    ...
    function one(pid, page) {
        ...
        var lowPrice = $("#lprice").val();
        var highPrice = $("#hprice").val();
        var str = "?pid="+pid+"&page="+page+"&pName="+pName+"&typeId="+typeId+"&lowPrice="+lowPrice+"&highPrice=" + highPrice;
        location.href = "${pageContext.request.contextPath}/prod/one.do" + str;
    }
</script>
<!--分页的AJAX实现-->
<script type="text/javascript">
    function ajaxsplit(page) {
        ...
        //异步ajax分页请求
        $.ajax({
        url:"${pageContext.request.contextPath}/prod/ajaxsplit.do",
            data:{"page":page,"pName":pName,"typeId":typeId,"lowPrice":lowPrice,"highPrice":highPrice},
            type:"post",
            success:function () {
                //重新加载分页显示的组件table
                //location.href---->http://localhost:8080/admin/login.do
                $("#table").load("http://localhost:8080/mimissm/admin/product.jsp #table");
